...
│ border-r   │  ├────────────────────────────────────┤  │
│ bg-sidebar │  │ content: px-5 py-6 lg:px-8         │  │
│            │  │  PageHeader                         │  │
│ [Brand]    │  │  本文（カード / テーブル / グリッド） │  │
│ ---------- │  │                                    │  │
│ Nav items  │  └────────────────────────────────────┘  │
│ ...        │                                          │
│ ---------- │                                          │
...
| メイン | `flex-1 overflow-auto` | — |
| コンテンツ余白 | `w-full px-5 py-6 lg:px-8` | misaki と同値 |

**旧案（ヘッダーバー + サイドナビ）は採用しない。** グローバルヘッダーは置かず、ブランド・ナビ・ログアウトはすべて左サイドバーに集約する（misaki と同じ）。

### 3.2 サイドバー詳細

| 要素 | 仕様 |
|------|------|
| ブランド行 | `border-b border-border px-4 py-3`。正方形マーク（`h-6 w-6 rounded-md bg-primary text-white text-[10px] font-bold`）+ アプリ名 `text-sm font-semibold` |
| アプリ名 | 社内: `談 業務システム` 等 `[要確認]`。施設: 同名または施設向け表示名 |
| ナビ | `flex flex-col gap-px px-2 py-2`。項目は `rounded-md px-2.5 py-1.5 text-[13px]` + lucide アイコン `h-4 w-4` |
| アクティブ | `bg-primary/8 font-medium text-primary` |
| 非アクティブ | `text-muted hover:bg-bg hover:text-text` |
| カテゴリ見出し | ナビ内で機能が多い場合、misaki の帳票サブナビと同様に `text-[11px] font-semibold uppercase tracking-wide text-muted` の見出しを挟む |
| 下部 | `border-t`。現在ユーザー（社内は `【社員番号】氏名`）と通知への導線、ログアウト（`hover:bg-danger/5 hover:text-danger`） |
| バージョン | サイドバー最下部に `text-xs text-muted` で単一バージョンを表示（A-04 解消） |
...
| 成り代わりバー | なし | あり |
| バージョン表示 | なし | サイドバー下部に表示 |
| 高密度グリッド | なし | 発注・注文で 12px 例外 |
| 通知 | なし | サイドバー下部またはアイコン導線 |
